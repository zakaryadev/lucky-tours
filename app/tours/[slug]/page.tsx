...
import { useState, FormEvent, useEffect, useCallback, ViewTransition } from "react";
import {
  ArrowLeft,
  ArrowRight,
  Calendar,
...
export default function TourDetailPage() {
  ...
  const tour = toursList.find((item) => item.slug.toLowerCase() === slug?.toLowerCase());
  ...
  const prevImage = useCallback(() => {
    if (!tour) return;
    setActiveImageIdx((prev) => (prev - 1 + tour.gallery.length) % tour.gallery.length);
  }, [tour]);
  ...
  if (!tour) {
    return (
      <div>
        <SiteHeader />
        <div className="shell section" style={{ minHeight: "60vh", paddingTop: "120px" }}>
          <h2>Tur topilmadi / Tour not found</h2>
          <p>Kechirasiz, siz qidirgan tur topilmadi.</p>
          <Link href="/tours" className="primary-button" style={{ marginTop: "20px" }}>
  ...
  const otherTours = toursList.filter((item) => item.slug !== tour.slug).slice(0, 3);
      ...
      <div className="tour-detail-wrapper">
        <div className="tour-detail-hero-bg">
          ...
          <div className="shell tour-detail-hero">
            <Link href="/tours" className="back-link" transitionTypes={["nav-back"]}>
              <ArrowLeft size={16} /> {t("detail.back")}
            </Link>
            ...
            <div className="tour-quick-info">
              ...
              <div className="info-chip">
                ...
                <span>
                  <strong>{t("detail.rating")}:</strong> {tour.rating} ({tour.reviewsCount})
                </span>
              </div>
              <div className="info-chip">
                <ShieldCheck size={16} />
        ...
        <main className="shell section tour-detail-content">
          ...
          <div className="interactive-gallery-container">
            ...
            >
              <ViewTransition name={`tour-${tour.slug}`} share="morph" default="none">
                <Image
                  src={mainImage}
                  alt={tour.title[lang]}
                  fill
              ...
              <div className="gallery-hover-overlay">
                <span className="expand-pill">
                  <Expand size={16} /> Rasmlarni ko&#39;rish ({galleryImages.length})
                </span>
              </div>
            </div>
            ...
            {activeTab === "overview" ? (
              <div className="tab-content">
                ...
                <section className="detail-card-block">
                  ...
                  <p className="detail-description">{tour.description[lang]}</p>
                </section>
            ...
            <div className="booking-card-sticky">
              ...
              {!bookingSent ? (
                <form className="sidebar-form" onSubmit={handleBooking}>
                  ...
                  <p className="form-subtext">Menejerimiz 10 daqiqa ichida siz bilan bog&#39;lanadi.</p>

                  <label>
                    <span>Ismingiz</span>
                    <input type="text" required placeholder="Masalan: Azizbek" />
                  </label>
                ...
                <div className="booking-success-box">
                  ...
                  <p>Tez orada menejerimiz siz bilan bog&#39;lanib, barcha ma&#39;lumotlarni beradi.</p>
                  <button
                    type="button"
              ...
              <div className="manager-contact-links">
                ...
                <a href="tel:+998712000001" className="contact-chip">
                  <Phone size={15} /> +998 71 200 00 01
                </a>
                <a href="https://t.me/luckytours" target="_blank" rel="noopener noreferrer" className="contact-chip telegram">
            ...
            {otherTours.map((item) => (
              <article key={item.slug} className="tour-card-full">
                <div className="tour-card-image">
                  <ViewTransition name={`tour-${item.slug}`} share="morph" default="none">
                    <Image
                      src={item.image}
                      alt={item.title[lang]}
                      fill
